                ...
                script {
                    env.K8S_VERSION = sh script: '''
                        if [ "${ghprbCommentBody}" != "" ]; then
                            python ${TESTDIR}/get-gh-comment-info.py ${ghprbCommentBody} --retrieve="k8s_version" | \
                    ...
                    env.KERNEL = sh script: '''
                        if [ "${ghprbCommentBody}" != "" ]; then
                            python ${TESTDIR}/get-gh-comment-info.py ${ghprbCommentBody} --retrieve="kernel_version" | \
                    ...
                    env.FOCUS = sh script: '''
                        if [ "${ghprbCommentBody}" != "" ]; then
                            python ${TESTDIR}/get-gh-comment-info.py ${ghprbCommentBody} --retrieve="focus" | \
                            sed "s/^$/K8s/" | \
